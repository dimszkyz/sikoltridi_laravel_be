import React, { useRef, useState } from "react";
import axios from "axios";

const API_BASE = import.meta.env.VITE_API_BASE_URL || "https://sikoltridi.sidome.id";
// PERBAIKAN 1: Gunakan endpoint jamak 'videos' sesuai api.php
const CREATE_ENDPOINT = `${API_BASE}/api/videos`;

export default function AddVideo({ open, onClose, onSuccess }) {
  const [judul, setJudul] = useState("");
  const [keterangan, setKeterangan] = useState("");
  const [media, setMedia] = useState(null);
  const [thumbnail, setThumbnail] = useState(null);
  const [submitting, setSubmitting] = useState(false);

  const mediaRef = useRef(null);
  const thumbRef = useRef(null);

  if (!open) return null;

  const resetForm = () => {
    setJudul("");
    setKeterangan("");
    setMedia(null);
    setThumbnail(null);
    if (mediaRef.current) mediaRef.current.value = "";
    if (thumbRef.current) thumbRef.current.value = "";
  };

  const onSubmit = async (e) => {
    e.preventDefault();
    if (!judul.trim()) return alert("Judul wajib diisi.");
    if (!media) return alert("File video wajib diunggah.");

    const fd = new FormData();
    fd.append("judul", judul.trim());
    fd.append("keterangan", keterangan.trim());
    fd.append("video_file", media); // PERBAIKAN NAMA FIELD: Sesuaikan dengan controller (video_file)
    if (thumbnail) fd.append("thumbnail_file", thumbnail); // PERBAIKAN NAMA FIELD: Sesuaikan dengan controller

    setSubmitting(true);
    try {
      // PERBAIKAN 2: Ambil Token
      const token = localStorage.getItem("token");
      if (!token) {
        alert("Sesi habis atau Anda belum login. Silakan login ulang.");
        setSubmitting(false);
        return;
      }

      // PERBAIKAN 3: Kirim Header Authorization
      await axios.post(CREATE_ENDPOINT, fd, {
        headers: {
          "Content-Type": "multipart/form-data",
          "Authorization": `Bearer ${token}`,
        },
      });

      resetForm();
      onClose?.();
      onSuccess?.();
      alert("Video berhasil diunggah!");
    } catch (err) {
      console.error(err);
      const msg = err.response?.data?.message || "Gagal mengunggah video.";
      alert(msg);
    } finally {
      setSubmitting(false);
    }
  };

  return (
    <div className="fixed inset-0 z-50">
      <div
        className="absolute inset-0 bg-black/50"
        onClick={() => {
          resetForm();
          onClose?.();
        }}
      />
      <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white shadow-xl">
        <div className="border-b px-6 py-4 flex items-center justify-between">
          <h2 className="text-lg font-medium">Upload Video</h2>
          <button
            onClick={() => {
              resetForm();
              onClose?.();
            }}
            className="text-gray-500 hover:text-gray-700 text-xl leading-none"
          >
            ×
          </button>
        </div>

        <form onSubmit={onSubmit} className="p-6 grid gap-5">
          <div className="grid gap-2">
            <label className="text-sm font-medium">Judul</label>
            <input
              type="text"
              placeholder="Masukkan judul video"
              value={judul}
              onChange={(e) => setJudul(e.target.value)}
              className="w-full rounded border px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>

          <div className="grid gap-2">
            <label className="text-sm font-medium">Keterangan</label>
            <textarea
              placeholder="Tambahkan deskripsi singkat video"
              value={keterangan}
              onChange={(e) => setKeterangan(e.target.value)}
              className="w-full rounded border px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500"
              rows={3}
            />
          </div>

          <div className="grid gap-2">
            <label className="text-sm font-medium">Upload Video (MP4)</label>
            <input
              ref={mediaRef}
              type="file"
              accept="video/mp4"
              onChange={(e) => setMedia(e.target.files?.[0] || null)}
              className="w-full rounded border px-3 py-2 file:mr-3 file:rounded file:border-0 file:bg-gray-100 file:px-3 file:py-2"
            />
          </div>

          <div className="grid gap-2">
            <label className="text-sm font-medium">
              Upload Thumbnail (JPG/PNG)
              <span className="ml-2 font-normal text-gray-500">(tidak wajib diisi)</span>
            </label>
            <input
              ref={thumbRef}
              type="file"
              accept="image/*"
              onChange={(e) => setThumbnail(e.target.files?.[0] || null)}
              className="w-full rounded border px-3 py-2 file:mr-3 file:rounded file:border-0 file:bg-gray-100 file:px-3 file:py-2"
            />
          </div>

          <div className="flex items-center gap-3 pt-2">
            <button
              type="submit"
              disabled={submitting}
              className="px-5 py-2 rounded bg-emerald-500 text-white hover:bg-emerald-600 disabled:opacity-60"
            >
              {submitting ? "Mengunggah…" : "Upload"}
            </button>
            <button
              type="button"
              onClick={() => {
                resetForm();
                onClose?.();
              }}
              className="px-5 py-2 rounded bg-gray-500 text-white hover:bg-gray-600"
            >
              Batal
            </button>
          </div>
        </form>
      </div>
    </div>
  );
}